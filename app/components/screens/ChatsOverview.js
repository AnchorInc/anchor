  ...
  render() {
    return (
      <View style={{ backgroundColor: 'white', width, height }}>
        <Header title='Chats' />
        <ScrollView>
          <ChatDetail onPress={() => this.navigateChatScreen(this.state.person)} displayName={this.state.person.displayName} unread={this.state.person.unread} imageURL={this.state.person.imageURL} lastTimestamp={this.state.person.lastTimestamp} lastMessage={this.state.person.lastMessage} />
